...
function exportPDF(plan, form) {
  const html = `
    <html>
    <head>
      <style>
        body { font-family: 'Georgia', serif; max-width: 680px; margin: 40px auto; color: #1a1a2e; line-height: 1.8; font-size: 14px; }
        .header { border-bottom: 3px solid #f472b6; padding-bottom: 16px; margin-bottom: 24px; }
        .header h1 { font-size: 22px; margin: 0 0 4px; color: #1a1a2e; }
        .meta { font-size: 12px; color: #666; }
        h2 { font-size: 17px; color: #c026a0; margin: 1.5rem 0 6px; border-left: 3px solid #f472b6; padding-left: 10px; }
        h3 { font-size: 15px; color: #1a1a2e; margin: 1rem 0 4px; }
        h4 { font-size: 14px; margin: 0.75rem 0 4px; }
        strong { color: #c026a0; }
        table { width: 100%; border-collapse: collapse; margin: 12px 0; font-size: 13px; }
        th { background: #fdf2f8; padding: 8px; text-align: left; border: 1px solid #f0abda; }
        td { padding: 8px; border: 1px solid #f0abda; }
        li { margin: 4px 0; }
        .footer { margin-top: 40px; padding-top: 12px; border-top: 1px solid #f0abda; font-size: 11px; color: #999; text-align: center; }
      </style>
    </head>
    <body>
      <div class="header">
        <h1>SAT Lesson Plan</h1>
        <div class="meta">
          Student: <strong>${form.student_name}</strong> &nbsp;|&nbsp;
          Section: ${form.section} &nbsp;|&nbsp;
          Duration: ${form.duration_mins} min &nbsp;|&nbsp;
          Date: ${new Date().toLocaleDateString()}
          ${form.current_score ? `&nbsp;|&nbsp; Score: ${form.current_score} → ${form.target_score}` : ''}
        </div>
      </div>
      ${plan
        .replace(/^# (.*?)$/gm, '<h2>$1</h2>')
      ...
      <div class="footer">Generated by TutorAI · ${new Date().toLocaleDateString()}</div>
    </body>
    </html>
  `
...
function LessonPlanner({ students, onRefresh }) {
  ...
  function renderMarkdown(text) {
    ...
    return text
      ...
      .replace(/^#### (.*?)$/gm, '<h5 style="font-size:13px;font-weight:500;margin:0.5rem 0 4px;color:var(--muted)">$1</h5>')
      ...
      {(plan || loading) && (
        <Card>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--muted)' }}>Generated lesson plan</div>
            {plan && !loading && (
              <button onClick={() => exportPDF(plan, form)} style={{ fontSize: 12, padding: '4px 12px', borderRadius: 6, border: '1px solid var(--border2)', background: 'transparent', color: 'var(--accent)', cursor: 'pointer' }}>
                ↓ Export PDF
              </button>
            )}
          </div>
